...
include_once _PS_MODULE_DIR_ . '/eservice/tools/sdk/payments.php';
include_once _PS_MODULE_DIR_ . '/eservice/translations/statuses.php';

class EService extends PaymentModule
{
     ...
     * 
     ***/
    const ST_SHOW_IFRAME = "0";
    const ST_SHOW_REDIRECT = "0";
    const ST_SHOW_HOSTEDPAY = "0";
    
    const ST_SHOW_SANDBOX_FIELDS = "0";
    const ST_SHOW_LIVE_FIELDS = "0";
    
    const ST_EVO_CASHIER_URL_SANDBOX = "https://cashierui-apiuat.test.secure.eservice.com.pl/ui/cashier";
    const ST_EVO_JAVASCRIPT_URL_SANDBOX = "https://cashierui-apiuat.test.secure.eservice.com.pl/js/api.js";
    const ST_EVO_TOKEN_URL_SANDBOX = "https://apiuat.test.secure.eservice.com.pl/token";
    const ST_EVO_PAYMENT_URL_SANDBOX = "https://apiuat.test.secure.eservice.com.pl/payments";
   
    const ST_EVO_CASHIER_URL_LIVE = "https://cashierui-api.secure.eservice.com.pl/ui/cashier";
    const ST_EVO_JAVASCRIPT_URL_LIVE = "https://cashierui-api.secure.eservice.com.pl/js/api.js";
    const ST_EVO_TOKEN_URL_LIVE = "https://api.secure.eservice.com.pl/token";
    const ST_EVO_PAYMENT_URL_LIVE = "https://api.secure.eservice.com.pl/payments";
    //DEFAULT TO IFRAME
    // 1 - IFRAME
    // 0 - REDIRECT
    // 2 - HOSTED PAY
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('eService');
        $this->description = $this->l('Accepts payments by eService');
    ...
    public function displayForm()
    {
        $form['eservice'] = [
            ...
            $form['merchant'] = [
                'form' => [
                    'legend' => [
                        'title' => $this->l('eService Connection settings'),
                        'icon' => 'icon-cog'
                    ],
                    'input' => [
                        [
        ...
        $this->addIntegrationMode($form['eservice']['form']['input']);
    ...
    public function buildTemplatePath($name)
    {
        ...
        return 'module:eservice/views/templates/front/' . $name . '17.tpl';
    }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $paymentOption->setCallToActionText($this->l('Pay with eService'))
            ...
            if ($orders) {
                foreach ($orders as &$orderr) {
                    ...
                    if($rawInfo !== ""){
                        ...
                        $orderr['info'] = $this->l('Refunded: ', 'eservice').$rawInfo;
                    }
    ...
    private function refundProcess($order, $tokenEvo)
    {
        ...
        if($possibleToRefund < 0 || round($refundAmount,2) > round($possibleToRefund,2)){
            return ['state' => 'danger', 'text' => $this->l('You can not make a refund on this amount for this order.', 'eservice')];
        }
        ...
        if(!$refund->errors){
            ...
            return ['state' => 'danger', 'text' => $this->l('No connect with payment gateway.', 'eservice')];
        }
        return false;
    }
    ...
    private function getSumCurrentOrderPayments($order)
    {
        ...
        foreach ($payments as $payment) {
            if($payment->payment_method=='eService'){
    ...
    {
        $link = $this->context->link->getModuleLink('eservice', 'payment');
    ...
    {
        $paymentOptions = [
            'cta_text' => $this->l('Pay with eService'),
            'action' => $this->context->link->getModuleLink('eservice', 'payment'),
    ...
    public function getPaymentToken($merchantTxId, $retry, $mode, $amount='')
    {
        ...
        try {
            ...
            $notificationUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=eservice&controller=response&ipg=0&retry=' . $retry;
            $landingUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=eservice&controller=response&ipg=0&retry=' 
    ...
    public function addNewOrderState($state, $names)
    {
		...
		$order_state->module_name = 'eservice';
    ...
    /**
     * get payment status description by eService status
     */
